...
class DatasetDF(Dataset):
    def __init__(
        ...
        # DFが有効かチェック
        self.df_enable = preprocess_config["preprocessing"]["df"]["enable"]
    ...
    def __getitem__(self, idx):
        ...
        phone = np.array(text_to_sequence(self.text[idx], self.cleaners))  # 音素のテキストデータを数値に変換
    ...
    def process_meta(self, filename):
        ...
        メタデータを読み込む
        Args:
            filename (str): メタデータファイルの名前
        Returns:
            name (list): 名前のリスト
            speaker (list): 話者のリスト
            text (list): テキストのリスト
            raw_text (list): 生のテキストのリスト
        """
        with open(
    ...
    def reprocess(self, data, idxs):
        ...
        if self.df_enable:
            dfs = [data[idx]["df"] for idx in idxs]
            dfs = pad_1D(dfs)
